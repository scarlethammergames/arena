...
public class fireProjectile: MonoBehaviour {
	...
	void MoveControllable(){
		...
		if (_controller.useGamepad){
			...
			_controller.controlerLookDirection = GamePad.GetAxis(GamePad.Axis.RightStick, _padIndex);
		}else{
			_controller.controllerMoveDirection = new Vector2(Input.GetAxis("Horizontal"), -Input.GetAxis("Vertical"));
      _controller.controlerLookDirection = new Vector2(Mathf.Clamp(Input.GetAxis("Mouse X"), -1, 1), Mathf.Clamp(Input.GetAxis("Mouse Y"), -1, 1));
		}
