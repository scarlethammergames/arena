...
namespace UnitySampleAssets.Cameras
{
    public class FreeLookCamEdit : PivotBasedCameraRig
    {
        ...
        protected void Update()
        {
			...
			var dpad_y = CrossPlatformInputManager.GetAxis ("Xbox360ControllerDPadY");
			
			if (dpad_y < 0 && invert_timer > 1) {
				if(inverted)
					inverted = false;
				else
					inverted = true;
        ...
        private void HandleRotationMovement()
        {
			...
			var y = CrossPlatformInputManager.GetAxis("Xbox360ControllerRightY") * (inverted ? 1 : -1);
